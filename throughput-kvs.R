...
# Size == "1048576" | 
df1$Transport <- paste("", ifelse(df1$Transport == "partisan", "prototype", "baseline"))
...
ggplot(data = df2, aes(x = Concurrency, y = (Ops / 120), fill = Transport, group = Experiment, color = Experiment)) +
  geom_line(aes(linetype=Transport), size=1) + geom_point(size=2.5, aes(shape=Size)) + ylab("Ops/Second") + ylim(0, 150) +
...
ggsave("c:\\users\\chris\\github\\unir\\KVS1MSThroughput.png", dpi = 400)
...
ggsave("c:\\users\\chris\\github\\unir\\KVS20MSThroughput.png", dpi = 400)