...
df <- read.csv("c:\\users\\chris\\GitHub\\unir\\figure-5.csv", header = FALSE)
...
& (Size == 8388608 & Latency == "1" & is.element(Concurrency, c(32, 64, 128))))
...
ggplot(data = df1, aes(x = Concurrency, y = Time, fill = Experiment)) +
  ...
  ggtitle("8MB Payload, 1ms RTT Latency, Queue Overhead") +
  theme(legend.position = c(0.2, 0.8), 
        legend.background = element_rect(color = "black", fill = "grey90", size = 1, linetype = "solid"))
ggsave("c:\\users\\chris\\github\\unir\\QueueOverhead.pdf")
