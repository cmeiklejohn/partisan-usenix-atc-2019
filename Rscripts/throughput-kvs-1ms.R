...
df <- read.csv("../throughput-kvs-1-1ms.csv", header = FALSE)
...
& (Latency == "1") & (Size == "1024" | Size == "8388608" | Size == "524288") & is.element(Concurrency, c(16, 32, 64, 128)))
...
for(n in c(2, 3, 4, 5)) {
  df3 <- read.csv(paste("../throughput-kvs-", toString(n), "-1ms.csv", sep=""), header = FALSE)
...
ggplot(data = final, aes(x = Concurrency, y = (Mean / 120), fill = Transport, group = Experiment, color = Experiment)) +
  geom_line(aes(linetype=Transport), size=1) + geom_point(size=2.5, aes(shape=Size)) + ylab("Ops/Second") + ylim(50, 150) +
  ...
  ggtitle("1/512/8192KB, 1ms RTT Latency, KVS Throughput")

ggsave("../KVS1MSThroughput-Final.png", dpi = 400)
