...
df <- read.csv("../throughput-echo-1-1ms.csv", header = FALSE)
...
for(n in c(2, 3, 4, 5)) {
  df3 <- read.csv(paste("../throughput-echo-", toString(n), "-1ms.csv", sep=""), header = FALSE)
...
ggplot(data = final, aes(x = Concurrency, y = (Mean / 120), fill = Transport, group = Experiment, color = Experiment)) +
  ...
  ggtitle("1/512/8192KB, 1ms RTT Latency, Echo Throughput")

ggsave("../Echo1MSThroughput-Final.png", dpi = 400)