...
df <- read.csv("../hroughput-kvs-1-20ms.csv", header = FALSE)
...
for(n in c(2, 3, 4, 5)) {
  df3 <- read.csv(paste("../throughput-kvs-", toString(n), "-20ms.csv", sep=""), header = FALSE)
...
ggplot(data = final, aes(x = Concurrency, y = (Mean / 120), fill = Transport, group = Experiment, color = Experiment)) +
  ...
  ggtitle("1/512/8192KB, 20ms RTT Latency, KVS Throughput")

ggsave("../KVS20MSThroughput-Final.png", dpi = 400)
